      ...
      {popularPosts && popularPosts.length > 0 && (
        <div className="p-6 sm:p-8 bg-white rounded-3xl border border-zinc-100 shadow-sm">
          <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
            <TrendingUp size={18} className="text-zinc-900" /> Popular Posts
          </h3>
          <div className="space-y-6">
            {popularPosts.slice(0, 5).map((post, index) => (
              ...
              >
                <span className="text-2xl font-black text-zinc-100 group-hover:text-zinc-500 transition-colors">
                  {index + 1}
                </span>
                <div>
      ...
      {tags.length > 0 && (
        <div className="p-6 sm:p-8 bg-white rounded-3xl border border-zinc-100 shadow-sm">
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
            <Tag size={18} className="text-zinc-900" /> Popular Tags
          </h3>
          <div className="flex flex-wrap gap-2">
            {tags.slice(0, 10).map((tag) => (
              <Link
                key={tag}
                to={`/tag/${tag}`}
                className="text-xs px-3 py-1.5 bg-zinc-100 hover:bg-zinc-100 hover:text-primary rounded-full font-medium transition-all"
              >
                {tag}
              </Link>
            ))}
