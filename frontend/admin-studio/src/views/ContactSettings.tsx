...
export const ContactSettings = () => {
  ...
  if (isLoading) {
    return <div className="py-12 text-center text-slate-500">Loading contact settings...</div>;
  }
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
            <Mail size={20} className="text-indigo-600" /> Contact Information
          </h2>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                required
              />
              <p className="text-xs text-slate-500 mt-1">Primary email address shown on the contact page.</p>
            </div>
            <div>
              ...
              />
              <p className="text-xs text-slate-500 mt-1">Optional phone number for direct contact.</p>
            </div>
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
              />
              <p className="text-xs text-slate-500 mt-1">City, region, or remote availability for visitors.</p>
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Expected Response Time</label>
              <input
                type="text"
                ...
                className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
              />
              <p className="text-xs text-slate-500 mt-1">Set expectations for how quickly you usually reply.</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
              />
              <span className="text-sm font-medium text-slate-700">Show FAQ on Contact page</span>
            </label>
          </div>
          ...
          {formData.show_faq && (
            <>
              <button
                type="button"
                onClick={addFaq}
                className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-lg font-bold hover:bg-indigo-700 transition-all mb-6"
              >
              ...
              <div className="space-y-4">
                {formData.faqs.map((faq, index) => (
                  <div key={index} className="flex gap-4 rounded-xl border border-slate-200 p-5">
                    <div className="flex-1 space-y-3">
                      ...
                      <textarea
                        ...
                        placeholder="Answer"
                        rows={3}
                        className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none resize-y"
                      />
                    </div>
                    <button
                      type="button"
        ...
        <button
          ...
          className="sticky bottom-0 w-full flex items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
