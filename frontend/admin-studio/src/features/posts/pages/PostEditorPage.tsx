...
export const PostEditor: React.FC = () => {
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('ðŸ–¼ï¸ Thumbnail upload:', file.name);
  ...
  const markdownComponents: Components = {
    ...
    a: ({ node, ...props }) => <a className="text-zinc-900 underline hover:text-zinc-950 dark:text-zinc-900 dark:hover:text-zinc-500" {...props} />,
    ...
    <div className="admin-page mx-auto max-w-5xl px-4 py-4 sm:p-6">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <button 
            ...
            className="admin-card flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800 sm:w-auto"
          >
      ...
      <div className="space-y-6">
        ...
        <input 
          ...
          className="w-full border-none bg-transparent text-3xl font-extrabold text-zinc-900 placeholder:text-zinc-400 focus:ring-0 dark:text-white dark:placeholder:text-zinc-500 sm:text-4xl" 
        />

        {/* Featured Thumbnail Section */}
        <div className="group relative flex flex-col items-start gap-4 rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 p-4 transition-colors hover:border-zinc-300 dark:border-zinc-300/30 dark:from-zinc-900/40 dark:to-zinc-900 dark:hover:border-zinc-300/50 sm:flex-row sm:items-center">
          <div className="relative h-24 w-36 shrink-0 overflow-hidden rounded-xl border border-zinc-300 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            {thumbnail ? (
              ...
              <div className="h-full w-full flex items-center justify-center">
                <ImageIcon className="text-zinc-300 dark:text-zinc-500" size={28} />
              </div>
            )}
            {isUploading && (
          ...
          <div className="min-w-0 flex-1 self-stretch">
            <label className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-zinc-900 dark:text-zinc-900">
              <LinkIcon size={10} /> Featured Thumbnail
            </label>
            <input 
              {...register('thumbnail_url')} 
              placeholder="Paste Cloudinary URL or upload below..." 
              className="mb-2 w-full bg-transparent font-mono text-sm text-zinc-700 outline-none placeholder:text-zinc-500 dark:text-zinc-200 dark:placeholder:text-zinc-900/70" 
            />
            <div className="flex flex-wrap gap-2">
              <label className="cursor-pointer">
                ...
                />
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-primary text-white text-xs font-medium rounded hover:bg-purple-700 transition">
        ...
        <div className="admin-section space-y-4 rounded-2xl p-6">
          <div className="mb-4 flex items-center gap-2">
            <TagIcon size={18} className="text-zinc-900" />
            <h3 className="text-lg font-bold text-zinc-900 dark:text-white">Tags</h3>
            <span className="text-xs text-zinc-500 dark:text-zinc-400">({selectedTagIds.length} selected)</span>
          </div>
              ...
              {selectedTagIds.map(tagId => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-primary text-white text-sm font-medium rounded-full hover:bg-purple-700 transition-all"
                  >
                    {tag.name}
                    <X size={14} />
                  </button>
              ...
              {allTags
                ...
                .map((tag: any) => (
                  <button
                    ...
                    className="rounded-full bg-zinc-100 px-3 py-1.5 text-sm font-medium text-zinc-700 transition-all hover:bg-zinc-100 hover:text-primary dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-100 dark:hover:text-zinc-500"
                  >
                    {tag.name}
                  </button>
                ))}
          ...
          <div className="border-t border-zinc-200 pt-4 dark:border-zinc-700">
            ...
            <form onSubmit={handleCreateTag} className="flex flex-col gap-2 sm:flex-row">
              ...
              <button
                ...
                className="px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/60">
          ...
          <label className="flex items-center gap-2 cursor-pointer group">
            <input type="checkbox" {...register('is_project')} className="rounded border-zinc-300 text-zinc-900 focus:ring-primary dark:border-zinc-600 dark:bg-zinc-900" /> 
            <span className="text-sm font-medium text-zinc-700 transition group-hover:text-primary dark:text-zinc-300 dark:group-hover:text-zinc-500">Portfolio Project</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer group">
            <input type="checkbox" {...register('published')} className="rounded border-zinc-300 text-zinc-900 focus:ring-primary dark:border-zinc-600 dark:bg-zinc-900" /> 
            <span className="text-sm font-medium text-zinc-700 transition group-hover:text-primary dark:text-zinc-300 dark:group-hover:text-zinc-500">Live Visibility</span>
          </label>
          </div>
        </div>
        ...
        {/* Upload Status */}
        {isUploading && (
          <div className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-900 dark:border-zinc-300/30 dark:bg-zinc-100 dark:text-zinc-500">
        ...
        {isPreview ? (
          <div className="prose prose-zinc dark:prose-invert max-w-none overflow-auto rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm min-h-[500px] sm:p-8 lg:p-10 dark:border-zinc-700 dark:bg-zinc-900">
            <ReactMarkdown 
